...
import com.ytgld.chest_item.renderer.WidgetTypes;
...
public abstract class AdvancementWidgetMixin implements IAdvancementWidget {
    ...
    @Override
    public void seekingImmortals$draw(net.minecraft.client.gui.GuiGraphicsExtractor guiGraphics, int x, int y) {
        if (!this.display.isHidden() || this.progress != null && this.progress.isDone()) {
            ...
            guiGraphics.fakeItem(this.display.getIcon().create(), x + this.x + 8, y + this.y + 5);
        }
    ...
    @Override
    public void seekingImmortals$drawHover(net.minecraft.client.gui.GuiGraphicsExtractor guiGraphics, int x, int y, float fade, int width, int height) {
        ...
        if (!this.description.isEmpty()) {
            if (flag1) {
                guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, Identifier.fromNamespaceAndPath(Chestitem.MODID,"advancements/box"), i2, k - j2, this.width, j2);
            } else {
                guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, Identifier.fromNamespaceAndPath(Chestitem.MODID,"advancements/box"), i2, j, this.width, j2);
            }
        }
        ...
        int k2 = i2 + 5;
        if (flag) {
            this.extractMultilineText(guiGraphics, this.titleLines, k2, j + 9, -1);
            if (component != null) {
                guiGraphics.centeredText(font, component, x + this.x - j1, j + 9, -1);
            }
        } else {
            this.extractMultilineText(guiGraphics, this.titleLines, x + this.x + 32, j + 9, -1);
            if (component != null) {
                guiGraphics.centeredText(font, component, x + this.x + this.width - j1 - 5, j + 9, -1);
            }
        }

        if (flag1) {
            this.extractMultilineText(guiGraphics, this.description, k2, j - l + 1, -16711936);
        } else {
            this.extractMultilineText(guiGraphics, this.description, k2, k, -16711936);
        }
