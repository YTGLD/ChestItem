...
public class EndComingRenderer extends EntityRenderer<EndComing> {
                      ...
                      MultiBufferSource vertexConsumers)
    {
        ...
        for (int i = 1; i < entity.getTrailPositions().size(); i++){
            ...
            Handler.renderBlood(matrices.last(), vertexConsumers.getBuffer(MRender.LIGHTNING), adjustedPrevPos, adjustedCurrPos, alpha,alpha/3.3F);
            Handler.renderBlood(matrices.last(), vertexConsumers.getBuffer(MRender.LIGHTNING_Outline), adjustedPrevPos, adjustedCurrPos, alpha,alpha/3.3F);
        }
        matrices.popPose();
    }
    ...
    public void renderSphere1(@NotNull PoseStack matrices, @NotNull MultiBufferSource vertexConsumers, int light, float s) {
        {
            ...
            for (int i = 0; i < stacks; ++i) {
                ...
                for (int j = 0; j < slices; ++j) {
                    ...
                    vertexConsumer.addVertex(matrices.last().pose(), x0, y0, z0).setColor(1.0f, 0,0, 1.0f).setOverlay(OverlayTexture.NO_OVERLAY).setUv(0, 0).setUv2(light, light).setNormal(1, 0, 0);
                    vertexConsumer.addVertex(matrices.last().pose(), x1, y1, z1).setColor(1.0f, 0,0, 1.0f).setOverlay(OverlayTexture.NO_OVERLAY).setUv(0, 0).setUv2(light, light).setNormal(1, 0, 0);
                    vertexConsumer.addVertex(matrices.last().pose(), x2, y2, z2).setColor(1.0f, 0,0, 1.0f).setOverlay(OverlayTexture.NO_OVERLAY).setUv(0, 0).setUv2(light, light).setNormal(1, 0, 0);
                    vertexConsumer.addVertex(matrices.last().pose(), x3, y3, z3).setColor(1.0f, 0,0, 1.0f).setOverlay(OverlayTexture.NO_OVERLAY).setUv(0, 0).setUv2(light, light).setNormal(1, 0, 0);
                }
            }
        }
        {
            int stacks = 20; // 垂直方向的分割数
            int slices = 20; // 水平方向的分割数
            VertexConsumer vertexConsumer = vertexConsumers.getBuffer(MRender.LIGHTNING_Outline);
            for (int i = 0; i < stacks; ++i) {
                float phi0 = (float) Math.PI * ((i + 0) / (float) stacks);
                float phi1 = (float) Math.PI * ((i + 1) / (float) stacks);
