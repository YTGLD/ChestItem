...
import com.ytgld.chest_item.entity.state.SmallSunRenderState;
...
public class SmallSunRender extends EntityRenderer<@NotNull SmallSun, @NotNull SmallSunRenderState> {
    public SmallSunRender(EntityRendererProvider.Context p_173917_) {
        super(p_173917_);
    }

    @Override
    public boolean shouldRender(SmallSun livingEntity, Frustum camera, double camX, double camY, double camZ) {
        return true;
    }

    @Override
    public @NotNull SmallSunRenderState createRenderState() {
        return new SmallSunRenderState();
    }

    @Override
    public void submit(SmallSunRenderState renderState, PoseStack poseStack, SubmitNodeCollector collector, CameraRenderState cameraRenderState) {
        HandlerClient.showOutline = true;
        HandlerClient.doPass = true;

        SmallSun entity = renderState.entity;
        ...
        collector.submitCustomGeometry(poseStack,MRender.colorOutline(true),(pose, bufferSource) -> {
            if (entity.onGTimeAlpha>0) {
                renderSphere1(pose, bufferSource, 255, (float) Math.sqrt(entity.onGTime)/2, entity.onGTimeAlpha);
            }
        });
    ...
    private void setTrial(PoseStack.Pose matrices, SmallSun entity, VertexConsumer vertexConsumers) {
        for (int i = 1; i < entity.getTrailPositions().size(); i++){
            ...
            renderBlood(matrices, vertexConsumers, adjustedPrevPos, adjustedCurrPos, alpha,alpha/3f);
            renderBlood(matrices, vertexConsumers, adjustedPrevPos, adjustedCurrPos, alpha,alpha/3f);

        }
    }
    public void renderBlood(PoseStack.Pose poseStack, VertexConsumer vertexConsumer, Vec3 start, Vec3 end, float a, float r) {
    ...
    private void addSquare(VertexConsumer vertexConsumer, PoseStack.Pose poseStack, Vec3 up1, Vec3 up2, Vec3 down1, Vec3 down2, float alpha) {
    ...
    public void renderSphere1(@NotNull PoseStack.Pose matrices, @NotNull VertexConsumer vertexConsumer, int light, float s,float a) {
        ...
        for (int i = 0; i < stacks; ++i) {
            ...
            for (int j = 0; j < slices; ++j) {
                ...
                vertexConsumer.addVertex(matrices, x0, y0, z0).setColor(1.0f, 1.0f, 0.2f, a).setOverlay(OverlayTexture.NO_OVERLAY).setUv(0, 0).setUv2(light, light).setNormal(matrices,1, 0, 0);
                vertexConsumer.addVertex(matrices, x1, y1, z1).setColor(1.0f, 1.0f, 0.2f, a).setOverlay(OverlayTexture.NO_OVERLAY).setUv(0, 0).setUv2(light, light).setNormal(matrices,1, 0, 0);
                vertexConsumer.addVertex(matrices, x2, y2, z2).setColor(1.0f, 1.0f, 0.2f, a).setOverlay(OverlayTexture.NO_OVERLAY).setUv(0, 0).setUv2(light, light).setNormal(matrices,1, 0, 0);
                vertexConsumer.addVertex(matrices, x3, y3, z3).setColor(1.0f, 1.0f, 0.2f, a).setOverlay(OverlayTexture.NO_OVERLAY).setUv(0, 0).setUv2(light, light).setNormal(matrices,1, 0, 0);}
        }
    }

    @Override
    public void extractRenderState(SmallSun entity, SmallSunRenderState reusedState, float partialTick) {
