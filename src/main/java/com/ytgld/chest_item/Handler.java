        ...
        if (player instanceof IPlayer iPlayer) {
            ...
            player.openMenu(new SimpleMenuProvider(
                    (i, inventory, p_53126_) -> ChestMenu.threeRows(i, inventory,
                            iPlayer.chest_item$chestInventory()), Component.translatable("container.chest_item.chest")
            ));
        }
    }
    public static ItemStack getItem(Player player,int inSlot){
        if (player instanceof IPlayer iPlayer) {
            return iPlayer.chest_item$chestInventory().getItem(inSlot);
        }
