...
public record RendererFarm(Matrix3x2fStack pose, GuiRenderState guiRenderState, int color) {
    ...
    private void chest_item$innerBlit(RenderPipeline pipeline, Identifier atlas, int x0, int x1, int y0, int y1, float u0, float u1, float v0, float v1, int color) {
    ...
    private void chest_item$submitBlit(RenderPipeline pipeline, GpuTextureView atlasTexture, int x0, int y0, int x1, int y1, float u0, float u1, float v0, float v1, int color) {
        if (Minecraft.getInstance().screen != null) {
            this.guiRenderState.submitGuiElement(new GUIRenderState(pipeline, TextureSetup.singleTexture(atlasTexture, RenderSystem.getSamplerCache().getRepeat(FilterMode.NEAREST)), new Matrix3x2f(this.pose), x0, y0, x1, y1, u0, u1, v0, v1, color, Minecraft.getInstance().screen.getRectangle()));
        }
    }
}
