...
public class Crave extends MemoryBase {
    ...
    public static class  CraveTooltip extends BaseTooltip {
        ...
        @Override
        public void doText(ItemStack stack, @UnknownNullability Consumer<Component> tooltipComponents) {
            tooltipComponents.accept(Component.translatable("item.chest_item.crave_tooltip.string.1").setStyle(Style.EMPTY.withColor(color())));
            tooltipComponents.accept(Component.translatable("item.chest_item.crave_tooltip.string.2").setStyle(Style.EMPTY.withColor(color())));
        }
        @Override
                ...
                if (MemoryBase.hasMemory(player, "chest_item:crave_tooltip")) {
                    if (event.getItem().getUseAnimation() == ItemUseAnimation.EAT) {
                        if (!player.level().isClientSide()) {
                            ...
                            addEffect(player, MobEffects.STRENGTH, 600, 0, 1);
                            addEffect(player, MobEffects.SPEED, 400, 0, 2);
                            addEffect(player, MobEffects.HASTE, 820, 0, 2);
                            addEffect(player, MobEffects.RESISTANCE, 300, 0, 1);
        ...
        private static void tickCooldown(Player player,int time){
            ...
            if (player.tickCount % time == 1) {
                if (hasCooldown(player)) {
                    compoundTag.putInt(cooldown,compoundTag.getIntOr(cooldown, 0) - 1);
                }
            }
        }
        ...
        private static boolean hasCooldown(Player player){
            CompoundTag compoundTag = player.getPersistentData();
            if (compoundTag.getIntOr(cooldown, 0) > 1) {
                return true;
            }
            return false;
        }
