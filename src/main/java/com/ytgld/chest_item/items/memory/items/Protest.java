...
public class Protest extends MemoryBase {
    ...
    public void doText(ItemStack stack, @UnknownNullability List<Component> tooltipComponents) {
        tooltipComponents.add(Component.translatable("item.chest_item.protest.string.1").withStyle(ChatFormatting.GRAY));
    }
    ...
    public void doTextGive(ItemStack stack, List<Component> tooltipComponents) {
        tooltipComponents.add(Component.translatable("item.chest_item.protest.string.give").withStyle(ChatFormatting.GRAY));
    }
    public static class ProtestTooltip extends BaseTooltip {
        ...
        @Override
        public void doText(ItemStack stack, @UnknownNullability List<Component> tooltipComponents) {
            tooltipComponents.add(Component.translatable("item.chest_item.protest_tooltip.string.1").setStyle(Style.EMPTY.withColor(color())));
            tooltipComponents.add(Component.translatable("item.chest_item.protest_tooltip.string.2").setStyle(Style.EMPTY.withColor(color())));
        }
        @Override
                ...
                if (player.tickCount % 20 == 1){
                    ...
                    for (Mob mob : list) {
                        ...
                        if (mob.getTarget() instanceof Player me && me.is(player)){
                            ...
                            if (!mob.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN,200,2,false,false))){
                                mob.hurt(mob.damageSources().playerAttack(player),
