...
public class EyeBook  extends ItemBase implements IGold {
    ...
    public static void tick(ItemStackTickEvent event){
        ...
        for (int i = 0; i < chestInventory.getContainerSize(); i++) {
            ...
            if (stack.is(InitItems.EyeBook_)) {
                if (player.tickCount % 80 == 0) {
                    player.addEffect(new MobEffectInstance(MobEffects.NIGHT_VISION, 600, 0, false, false), player);
                    break;
                }
            }
        }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipAdder, TooltipFlag flag) {
        ...
        tooltipAdder.add(Component.translatable("item.chest_item.eye_book.string.0").withStyle(ChatFormatting.YELLOW).withStyle(ChatFormatting.ITALIC));
        tooltipAdder.add(Component.literal(""));
        tooltipAdder.add(Component.translatable("item.chest_item.eye_book.string.1").withStyle(ChatFormatting.GOLD));
    }
