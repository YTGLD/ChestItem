...
public class ArmorStone extends ItemBase implements IGold {
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipAdder, TooltipFlag flag) {
        ...
        tooltipAdder.add(Component.translatable("item.chest_item.armor_stone.string.0").withStyle(ChatFormatting.YELLOW).withStyle(ChatFormatting.ITALIC));
        tooltipAdder.add(Component.literal(""));
        tooltipAdder.add(Component.translatable("item.chest_item.armor_stone.string.1", 200-ConfigItem.intValue.get().floatValue()* 100F - 100f).withStyle(ChatFormatting.GOLD));
    }
