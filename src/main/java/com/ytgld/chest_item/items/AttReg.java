...
public class AttReg {
    ...
    public static final Supplier<AttachmentType<Set<String>>> itemRecord = ATTACHMENT_TYPES.register(
            ...
            () -> AttachmentType.<Set<String>>builder(() -> new HashSet<>())
                    ...
                    .serialize(TheMemoryDataHandler.StringSetCodec.CODEC.fieldOf("item_record").codec())
                    .build()
    );
    ...
    public static final Supplier<AttachmentType<Integer>> swordIntent = ATTACHMENT_TYPES.register(
            "sword_intent", () -> AttachmentType.builder(() -> 0).sync(new IntSyncHandler()).serialize(Codec.INT.
                    fieldOf("sword_intent").codec()).build()
    ...
    public static final Supplier<AttachmentType<Integer>> hyperplasiaCooldownAttachmentType = ATTACHMENT_TYPES.register(
            "hyperplasia_cooldown_data", () -> AttachmentType.builder(() -> 0).sync(new IntSyncHandler()).serialize(Codec.INT.
                    fieldOf("hyperplasia_cooldown_data").codec()).build()
    );
    /**
     * 失败的理论——探索未知
     */
