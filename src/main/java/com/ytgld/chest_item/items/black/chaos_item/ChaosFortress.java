...
public class ChaosFortress extends ItemBlackShadow implements ITheChaos {
    ...
    public static void isInvulnerableToBase(Player player,
        ...
        //被攻击时有5%的概率获得3秒无敌
        if (player.getCooldowns().isOnCooldown(InitItems.ChaosFortress_.asItem())) {
    ...
    public static Multimap<Holder<Attribute>, AttributeModifier> attributeModifierMultimap(Player player) {
        ...
        //侵蚀装甲难以自然恢复
        modifiers.put(AttReg.chaos_armor_speed, new AttributeModifier(ResourceLocation.parse(Chestitem.MODID +
                InitItems.ChaosFortress_.asItem().getDescriptionId()),
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipAdder, TooltipFlag flag) {
        ...
        if (compoundTag != null) {
            ...
            tooltipAdder.add((Component.translatable("item.chest_item.chaos_fortress.string.0")).withStyle(Style.EMPTY.withColor(TextColor.fromRgb(colorText()))));
        }
    }
    @Nullable
    @Override
