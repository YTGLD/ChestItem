...
public class Blood extends TheCelestial{
                ...
                if (!player.level().isClientSide()) {
                    for (int i = 0; i < chestInventory.getContainerSize(); i++) {
                        ...
                        if (stack.is(InitItems.Blood_)) {
                            ...
                            if (compoundTag != null) {
                                if (compoundTag.getInt(time) > 0) {
                                    event.setNewDamage(event.getNewDamage()*1.3f);
                                    player.heal(event.getNewDamage() * 0.2f);
                                    compoundTag.putInt(time,compoundTag.getInt(time)+20);
                                    break;
                                }
                            }
                        }
                ...
                if (!player.level().isClientSide()) {
                    for (int i = 0; i < chestInventory.getContainerSize(); i++) {
                        ...
                        if (stack.is(InitItems.Blood_)) {
                            ...
                            if (compoundTag != null) {
                                if (compoundTag.getInt(time) > 0) {
                                    compoundTag.putInt(time,compoundTag.getInt(time)-1);
                                    break;
                                }
                            }
                        }
    ...
    public void appendHoverText(ItemStack stack, TooltipContext context, List<Component> tooltipComponents, TooltipFlag tooltipFlag) {
       ...
       tooltipComponents.add(Component.translatable("item.chest_item.blood.string.1").withStyle(Style.EMPTY.withColor(TextColor.fromRgb(0X806A5ACD))).withStyle(ChatFormatting.ITALIC));
       tooltipComponents.add(Component.translatable("item.chest_item.blood.string.2").withStyle(Style.EMPTY.withColor(TextColor.fromRgb(0X806A5ACD))).withStyle(ChatFormatting.ITALIC));
       tooltipComponents.add(Component.translatable("item.chest_item.blood.string.3").withStyle(Style.EMPTY.withColor(TextColor.fromRgb(0X806A5ACD))).withStyle(ChatFormatting.ITALIC));
       tooltipComponents.add(Component.translatable("item.chest_item.blood.string.4").withStyle(Style.EMPTY.withColor(TextColor.fromRgb(0X806A5ACD))).withStyle(ChatFormatting.ITALIC));
    }
    @Override
    public ResourceLocation img(ItemStack stack) {
        return ResourceLocation.fromNamespaceAndPath(Chestitem.MODID,"textures/gui/soul/celestial/blood.png");
    }
