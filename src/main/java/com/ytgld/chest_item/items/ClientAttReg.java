...
public class ClientAttReg {
    ...
    public static final Supplier<AttachmentType<Set<String>>> record = ATTACHMENT_TYPES.register(
            "record",
            () -> AttachmentType.<Set<String>>builder(() -> new HashSet<>())
                    .sync(new TheMemoryDataHandler.StringSetSync())
                    .serialize(TheMemoryDataHandler.StringSetCodec.CODEC.fieldOf("record").codec())
                    .build()
    );
