 ...
 *
 */
public class Contingency extends ReinforcedBaseItem {
        ...
        if (event.getEntity() instanceof Player player) {
            if (ReinforcedBaseItem.hasReinforcedItem(player, ReinforcedItems.Contingency_.asItem())) {
                if (ReinforcedBaseItem.decayHeartIsZero(player)) {
                    if (!player.getCooldowns().isOnCooldown(ReinforcedItems.Contingency_.asItem().getDefaultInstance())){
                        Handler.addHeartShield(player,ConfigItem.intValue.getAsInt());
                        player.getCooldowns().addCooldown(ReinforcedItems.Contingency_.asItem().getDefaultInstance(), ConfigItem.intValue2.getAsInt());
                    }
                }
            }
        }
    ...
    public void text(ItemStack stack, Consumer<Component> tooltipComponents) {
        ...
        tooltipComponents.accept(Component.translatable("item.chest_item.contingency.string.0", ConfigItem.intValue.getAsInt(), ConfigItem.intValue2.getAsInt() / 20).withStyle(Style.EMPTY.withColor(color)));
    }
    @Override
    public int sanDown() {
        return -2;
