 ...
 */
public class Activity extends ReinforcedBaseItem {
        ...
        if (event.getEntity() instanceof Player player) {
            if (ReinforcedBaseItem.hasReinforcedItem(player, ReinforcedItems.Activity_.asItem())) {
                if (ReinforcedBaseItem.decayHeartIsZero(player)) {
                    if (!player.getCooldowns().isOnCooldown(ReinforcedItems.Activity_.asItem())){
                        ...
                        player.getCooldowns().addCooldown(ReinforcedItems.Activity_.asItem(),ConfigItem.intValue2.getAsInt() * 2);
                    }
                }
            }
        }
    ...
    public void text(ItemStack stack, List<Component> tooltipComponents) {
        ...
        tooltipComponents.add(Component.translatable("item.chest_item.activity.string.0",ConfigItem.intValue.getAsInt(),ConfigItem.intValue2.getAsInt() / 20).withStyle(Style.EMPTY.withColor(color)));
    }
