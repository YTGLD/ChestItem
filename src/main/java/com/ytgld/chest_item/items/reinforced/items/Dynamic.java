 ...
 */
public class Dynamic extends ReinforcedBaseItem {
        ...
        if (event.getEntity() instanceof Player player) {
            if (ReinforcedBaseItem.hasReinforcedItem(player, ReinforcedItems.Dynamic_.asItem())) {
                if (ReinforcedBaseItem.decayHeartIsZero(player)) {
                    if (!player.getCooldowns().isOnCooldown(ReinforcedItems.Dynamic_.asItem().getDefaultInstance())){
                        ...
                        player.getCooldowns().addCooldown(ReinforcedItems.Dynamic_.asItem().getDefaultInstance(), ConfigItem.intValue2.getAsInt());
                    }
                }
            }
        }
    ...
    public void text(ItemStack stack, Consumer<Component> tooltipComponents) {
        ...
        tooltipComponents.accept(Component.translatable("item.chest_item.dynamic.string.0",ConfigItem.intValue2.getAsInt() / 20).withStyle(Style.EMPTY.withColor(color)));
    }
    @Override
    public int sanDown() {
        return -2;
