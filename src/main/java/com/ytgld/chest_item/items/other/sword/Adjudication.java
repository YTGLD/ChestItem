package com.ytgld.chest_item.items.other.sword;
...
public class Adjudication extends ItemBase {
        ...
        if (event.getSource().getEntity() instanceof Player player) {
            if (Handler.has(player, InitItems.Adjudication_.asItem())) {
                ...
                if (!player.getCooldowns().isOnCooldown(InitItems.Adjudication_.asItem().getDefaultInstance())) {
                    ...
                    player.getCooldowns().addCooldown(InitItems.Adjudication_.asItem().getDefaultInstance(),number * timeBase);
                }
            }
        }
    }
    ...
    public void text(ItemStack stack, Consumer<Component> tooltipComponents, TooltipFlag flag) {
        ...
        tooltipComponents.accept(Component.translatable("item.chest_item.adjudication.string.0",ConfigItem.intValue.get() * (timeBase / 20)).withStyle(ChatFormatting.GOLD));
        tooltipComponents.accept(Component.translatable("item.chest_item.adjudication.string.1").withStyle(ChatFormatting.GOLD));
    }
}
